...
class salary {
  ...
  getSalary() {
    if (this.coachIncome < 100000)
      return this.baseBonus + 1.1 * this.coachIncome;
    if (this.coachIncome < 300000)
      return this.baseBonus + 1.15 * this.coachIncome;
  ...
  getMyCal() {
    return this.historyRecords.reduce((totalCalories, record) => {
      ...
      if (activity) {
        ...
        console.warn(
          `activities 沒有這個 id [${record.activities_id}] 喔 確認遺下!!!`
        );
      }
...
console.log(
  `${sportRecordsMin.name}今日一共消耗約 ${sportRecordsMin.getMyCal} 卡路里。`
);
...
function is533role() {
  ...
  const perWeekSportDuration = exerciseRecords.reduce(
  ...
  exerciseRecords.map((record) => {
    ...
    if (!role)
      console.log(
        `每次運動必須達到 **30 分鐘** 且平均心率達 **130 次以上** 才算合格。`,
        record
      );
  });
  return role;
